...
For each simulation, the migration graph will be saved in the respective output directory. To extract the pattern from the graph, we will use some helper functions which you can access `here <https://github.com/samsonweiner/sistem/blob/main/scripts/get_migration_pattern.py>`_. Each simulation may take 15-30 minutes, depending on parameters.
